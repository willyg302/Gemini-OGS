...
public class CreateAccount extends HttpServlet {
     ...
     * password 2 - invalid verification code 3 - account with that password
     * already exists
     */
    public static int accountCreator(String code, String pass, String email) throws UnsupportedEncodingException {
        int error = 0;
        if (Validate(pass)) {
            if (CheckUnique(pass)) {
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        int error = accountCreator(req.getParameter("name"), req.getParameter("email"), req.getParameter("verify"));
        if (error > 0) {
            resp.sendRedirect("/validate.jsp?verify=" + req.getParameter("verify") + "&error=" + error);
        } else {
